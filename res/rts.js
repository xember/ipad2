...
(function($){
	...
	$.fn.animateHighlight = function (highlightColor, duration) {
		...
		if( typeof($.ui) == 'undefined' || ($.browser.msie && $.browser.version.substring(0,$.browser.version.indexOf('.'))<8) ) {
			$(this).css("backgroundColor", highlightBg).animate({opacity:'+=0'},1000,function(e){$(this).css("backgroundColor", originalBg)});
		} else {
...
Date.prototype.format = function (sFormat) {
	...
	var lz = function(s) { return String(100 + Number(s)).substring(1); }
	var t = sFormat;
...
var rts = {
	jwsIdx: [0],
	jwsURLs: [["ws://localhost:8787/jWebSocket/jWebSocket"]],
	...
	init: function() {
		...
		$('body').data('stats',oStats); oStats = null;
			...
			if( this.jwsIdx != null && this.fallbackURLs == null ) {
				...
				if( this.fallbackURLs ){
					...
					if( this.sMode == "site" && this.sUsername ){
						...
						ajaxToken = {username:this.sUsername,password:this.sPassword,mode:this.sMode,object:this.sObject};
					}
					if( this.sMode == "site" && !this.sUsername ){
						this.log( "[init] Requesting anonymous fallback logon ...." );
						ajaxToken = {mode:this.sMode,object:this.sObject};
					}
					if( ajaxToken ){
	...
	exit: function() {
		...
		if( rts.bFallback ) {
			$.post(rts.fallbackURLs.logoff, {wsid:rts.sFallbackID});
		}
	},
	...
	poll: function() {
		$.get(rts.fallbackURLs.getdata, {wsid:rts.sFallbackID}, function(data) {
	...
	jwsLogon: function(sId){
		...
		$(rts.idMsgWrapper+" div.warning").remove();
		$(rts.idMsgWrapper+" div.error").remove();
		var jwsToken = {type:"user",subtype:"logon",role:null,clientversion:rts.VERSION,clientname:rts.NAME};
